...
const RISK_PATTERNS: ReadonlyArray<{ label: string; pattern: RegExp; question: string }> = [
  {
...
function detectStrongMatches(resume: string, vacancy: string): SkillEvidence[] {
  ...
  for (const [skill, aliases] of Object.entries(SKILL_ALIASES)) {
    if (containsAny(vacancy, aliases) && containsAny(resume, aliases)) {
      matches.push({ requirement: skill, evidence: `Resume contains evidence for ${skill}.`, confidence: 0.85 });
    }
  }
...
function positioningForMissingSkill(skill: string): string {
  if (skill === "Kafka") return "Do not claim Kafka experience unless true. Mention adjacent microservice or integration testing experience if supported.";
  if (skill === "Docker") return "Mention Docker basics only if there is real hands-on experience with containers or local environments.";
  if (skill === "Python") return "Frame Python as automation transition interest unless production automation experience is proven.";
...
function detectSalaryFit(vacancy: string): SalaryFit {
  if (!vacancy.includes("salary") && !vacancy.includes("compensation")) return "unknown";
  if (vacancy.includes("not specified") || vacancy.includes("competitive")) return "unknown";
